...
/** Every frequency the scheduler knows how to run. */
const BACKUP_FREQUENCIES: BackupScheduleConfig['frequency'][] = [
  'hourly',
  'every_2_hours',
  'every_3_hours',
  'every_4_hours',
...
if (persistedData) {
  ...
  // "after every new order" was removed because it never actually fired on an
  // order; it behaved as an hourly backup. Move anyone still on it to the
  // setting that matches what they were really getting, so the panel does not
  // show an empty dropdown and the daemon keeps a known interval.
  if (!BACKUP_FREQUENCIES.includes(backupSchedule.frequency)) {
    console.log(`[backup] frequency "${backupSchedule.frequency}" is no longer supported; using hourly.`);
    backupSchedule.frequency = 'hourly';
  }
...
async function startServer() {
  ...
  app.put('/api/backup/schedule', (req: Request, res: Response) => {
    ...
    // Reject a frequency the daemon cannot run, instead of storing it and
    // silently falling back to a daily backup the admin never chose.
    if (incoming.frequency !== undefined && !BACKUP_FREQUENCIES.includes(incoming.frequency)) {
      res.status(400).json({
        success: false,
        message: `دورهٔ زمانی «${incoming.frequency}» پشتیبانی نمی‌شود.`,
      });
      return;
    }
  ...
  // Helper to edit the current message in place or send a new one
  async function sendOrEditBotMessage(
    token: string,
    chatId: string,
    messageId: number | undefined,
    text: string,
    ...
    if (photo) {
      try {
        ...
        const pBody: any = await pRes.json().catch(() => null);
        if (pBody?.ok) return;
      } catch {
        // Fallback to sendMessage
      }
  ...
  // Interactive inline quantity picker with + and - glass buttons
  async function sendOrEditQuantityPicker(
    token: string,
    chatId: string,
    messageId: number | undefined,
    prod: any,
    ...
    text += `💎 <b>مبلغ کل:</b> <b>${totalPrice.toLocaleString('fa-IR')} تومان</b>\n\n`;
    ...
    await sendOrEditBotMessage(token, chatId, messageId, text, buttons, prod.image);
  }

  // Render and send the complete 5-button shopping cart overview (editing in place if messageId provided)
  async function sendBotCartView(token: string, chatId: string, messageId?: number) {
    ...
    if (update.message) {
      ...
      // longer handled; موعد تحویل is collected out-of-band by the workshop.)
        ...
        await sendOrEditBotMessage(
          ...
          chatId,
          messageId,
          `✅ <b>${quantity.toLocaleString('fa-IR')} ${prod.unit}</b> از «${prod.name}» به سبد خرید افزوده شد.\n\n🛒 <b>تعداد کل اقلام سبد:</b> ${totalQty.toLocaleString('fa-IR')}`,
          [
        ...
        const prod = products.find(p => p.id === prodId);
        if (!prod) return;
        await sendOrEditQuantityPicker(token, chatId, messageId, prod, 1);
      } else if (data.startsWith('custom_qty_')) {
        const prodId = data.replace('custom_qty_', '');
        const prod = products.find(p => p.id === prodId);
        if (!prod) return;
        await sendOrEditQuantityPicker(token, chatId, messageId, prod, 1);
      } else if (data.startsWith('pick_qty_')) {
        const parts = data.replace('pick_qty_', '').split('_');
        const prodId = parts[0];
        const newQty = parseInt(parts[1], 10) || 1;
        const prod = products.find(p => p.id === prodId);
        if (!prod) return;
        await sendOrEditQuantityPicker(token, chatId, messageId, prod, newQty);
      } else if (data === 'noop_qty') {
        await fetch(`https://api.telegram.org/bot${token}/answerCallbackQuery`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ callback_query_id: cb.id, text: 'برای تغییر تعداد از دکمه‌های ➕ و ➖ استفاده کنید.' })
        }).catch(() => {});
      } else if (data.startsWith('confirm_pick_')) {
        ...
        let cartTotal = 0;
        for (const item of cart) {
          const p = products.find(prodItem => prodItem.id === item.productId);
          if (p) {
        ...
        // Quantity > 1 -> Only full removal and custom quantity removal
        const qtyButtons: any[][] = [
          [{ text: `🗑️ حذف کامل محصول (${item.quantity.toLocaleString('fa-IR')} ${prod.unit})`, callback_data: `cart_rem_qty_${prod.id}_all`, style: 'danger' }],
        ...
        // The list is only a list: one glass button per order labelled with
        // its order code. All the details live behind that button, so the
        // customer is not handed every order as a wall of text.
        const summary = [
          userOrders.length ? `${userOrders.length} سفارش عادی` : '',
          userCustomOrders.length ? `${userCustomOrders.length} سفارش سفارشی` : '',
        ].filter(Boolean).join(' و ');

        const listKeyboard: any[][] = [];
        for (const ord of userOrders) {
          listKeyboard.push([{
            text: `🧾 ${ord.orderNumber}`,
            callback_data: `track_one_${ord.id}`,
          }]);
        }
        for (const customOrder of userCustomOrders) {
          listKeyboard.push([{
            text: `🎂 ${customOrder.orderNumber}`,
        ...
        // The full detail, exactly as it used to be shown for every order.
        let orderText = `🔖 <b>کد سفارش:</b> <code>${ord.orderNumber}</code>\n`;
        orderText += `📊 وضعیت: <b>${orderStatusLabel(ord.status)}</b>\n`;
        orderText += `🕑 ثبت: ${formatIranianDateTime(ord.createdAt)}\n\n`;
        orderText += `📦 <b>اقلام:</b>\n`;
        ord.items.forEach((item, idx) => {
          orderText += `${idx + 1}. ${escapeTelegramHtml(item.productName)}\n`;
          orderText += `   کد: <code>${escapeTelegramHtml(item.productCode || '---')}</code>\n`;
          orderText += `   ${item.quantity} ${escapeTelegramHtml(item.unit || '')} × ${item.price.toLocaleString()} = <b>${(item.price * item.quantity).toLocaleString()}</b>\n\n`;
        });
        orderText += `─────────────────\n`;
