...
function buildProductCard(prod: any, inCartQty: number) {
  ...
  cap += `💰 <b>قیمت:</b> ${priceText}${prod.unit && prod.unit !== 'کیلوگرم' ? ` / ${escapeHtml(prod.unit)}` : ''}\n`;
  cap += `📦 <b>وضعیت:</b> ${prod.isAvailable ? '🟢 موجود و تازه' : '🔴 ناموجود'}\n`;

  if (inCartQty > 0) {
    const lineTotal = effectivePrice * inCartQty;
    cap += `\n🛒 <b>تعداد در سبد شما:</b> <b>${inCartQty} ${escapeHtml(prod.unit && prod.unit !== 'کیلوگرم' ? prod.unit : 'عدد')}</b> (جمع: <b>${lineTotal.toLocaleString('fa-IR')} تومان</b>)\n`;
  }
...
export async function handleCustomerCallback(ctx: TelegramContext, data: string): Promise<boolean> {
  ...
  if (data.startsWith('cat_')) {
    ...
    for (const prod of filtered.slice(0, 10)) {
      ...
      const itemInCart = cart.find((i: any) => i.productId === prod.id);
  ...
  if (data.startsWith('add_to_cart_') || data.startsWith('inc_cart_') || data.startsWith('inc5_cart_') || data.startsWith('add_qty_')) {
    ...
    if (data.startsWith('inc5_cart_')) {
      ...
      prodId = data.replace('add_to_cart_', '').replace('inc_cart_', '');
      qtyToAdd = 1;
    }
  ...
  if (data.startsWith('dec_cart_')) {
    ...
    if (existing) {
      ...
      if (existing.quantity <= 0) {
        const newCart = cart.filter((i: any) => i.productId !== prod.id);
        ctx.userCarts.set(ctx.chatId, newCart);
        inCartQty = 0;
      } else {
        inCartQty = existing.quantity;
        ctx.userCarts.set(ctx.chatId, cart);
...
export async function handleAdminCallback(ctx: TelegramContext, data: string): Promise<boolean> {
  ...
  if (data.startsWith('admin_delete_prod_')) {
    ...
    if (idx !== -1) {
      const n = ctx.products[idx].name;
      ctx.products.splice(idx, 1);
      await notifyForumTopic(
        ctx,
        'products',
  ...
  if (data.startsWith('admin_cstatus_')) {
    ...
    const customLabels: any = { baking: '👩‍🍳 در حال پخت', ready: '📦 آماده تحویل', delivered: '✅ تحویل شد', rejected: '❌ رد شد' };
    await notifyForumTopic(
      ctx,
      'custom_orders',
  ...
  if (data.startsWith('admin_del_disc_')) {
    ...
    if (idx !== -1) {
      const removedCode = ctx.discounts[idx].code;
      ctx.discounts.splice(idx, 1);
      await notifyForumTopic(
        ctx,
        'discounts',
